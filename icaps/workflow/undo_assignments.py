...
def process_cmd_line_args() -> Namespace:
    ...
    """
    parser: ArgumentParser = ArgumentParser(description="Scrubs an assignment undoing all assignments")
    parser.add_argument("--input-file",
                        type=str,
                        default=None,
                        required=True,
                        help="Input CSV file containing the assignment data")
    parser.add_argument("--only-edges",
                        action='store_true',
                        help="Only deletes edges between papers and reviewers")
...
def main(opt: Namespace) -> None:
    ...
    client.delete_edges(invitation=f'{venue_id}/{role_id}/-/Assignment', wait_to_finish=True)
    if opt.only_edges:
        return
    ...
    for _, data in tqdm(assignments.iterrows()):
        ...
        client.remove_members_from_group(group=f'{venue_id}/Submission{number}/{role_id}', members=[data['rev.id']])
